    ...
    @hasrole('admin')
        <form action="{{ route('admin.student-achievement.update', [$achievement->id, $student->id]) }}" method="POST"
            ...
            <section class="w-full flex max-md:flex-col-reverse items-center md:space-x-3 md:justify-between text-white">
                ...
                <button type="submit"
                    class="outline-none w-full md:last:h-11 max-md:h-10 flex items-center justify-center font-medium bg-green-600 rounded shadow-sm transition duration-300 hover:bg-green-700 focus:bg-green-700 max-md:mb-2">Update</button>
            </section>
        </form>
    @endhasrole
