    ...
    @hasrole('teacher')
        ...
        <div class="w-full p-5 rounded-md shadow-lg bg-white mb-10 max-md:text-sm">
            <div class="flex md:items-center max-md:items-start md:h-12 max-md:min-h-11">
                ...
                <p class="w-full">{{ $achievement->student->name }}</p>
            </div>
            <div class="flex md:items-center max-md:items-start md:h-12 max-md:min-h-11">
                <p class="md:font-bold max-md:font-semibold w-44">Tanggal</p>
                <p class="w-5 font-bold">:</p>
                <p class="w-full">{{ \Carbon\Carbon::parse($achievement->created_at)->format('d-m-Y') }}</p>
            </div>
            <div class="flex md:items-center max-md:items-start md:min-h-12 max-md:min-h-11">
                <p class="md:font-bold max-md:font-semibold w-44">Pencapaian</p>
                <p class="w-5 font-bold">:</p>
                <p class="w-full">{{ $achievement->achievement }}</p>
            </div>
            @if ($achievement->created_at != $achievement->updated_at)
                <div class="flex md:items-center max-md:items-start md:h-12 max-md:min-h-11">
                    <p class="md:font-bold max-md:font-semibold w-44">Tanggal Update</p>
                    <p class="w-5 font-bold">:</p>
                    <p class="w-full">{{ \Carbon\Carbon::parse($achievement->updated_at)->format('d-m-Y, H:i:s') }}</p>
                </div>
                <div class="flex md:items-center max-md:items-start md:h-12 max-md:min-h-11">
                    <p class="md:font-bold max-md:font-semibold w-44">Diupdate Oleh</p>
                    <p class="w-5 font-bold">:</p>
                    <p class="w-full">{{ $achievement->updatedBy ? $achievement->updatedBy->name : '-' }}</p>
                </div>
            @endif
        </div>
    @endhasrole
