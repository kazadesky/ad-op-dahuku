...
@section('content')
    @hasrole(['super_admin', 'admin'])
        <form
            action="{{ route(Auth::user()->hasRole('super_admin') ? 'sa.teacher.update' : 'admin.teacher.update', $teacher->id) }}"
            method="POST" class="w-full md:p-10 max-sm:p-6 rounded-lg bg-white text-hitam shadow-lg max-md:text-sm">
            @csrf
            @method('PATCH')
            ...
            <section class="w-full md:mb-4 max-md:mb-3">
                <div class="flex max-md:flex-col max-md:space-y-1 md:space-x-4 md:items-center">
                    ...
                    <select name="teacher_status" id="status"
            ...
            <section class="w-full flex max-md:flex-col-reverse items-center md:space-x-3 md:justify-between text-white">
                <a href="{{ route(Auth::user()->hasRole('super_admin') ? 'sa.teacher.index' : 'admin.teacher.index') }}"
