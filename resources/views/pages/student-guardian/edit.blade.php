...
@section('subtitle')
    <p class="flex items-center space-x-px capitalize text-white">
        ...
        @hasrole('admin')
            <span>Admin</span>
        @endhasrole
        <span>/ Page / Akun Wali Santri / Edit</span>
    </p>
@endsection

@section('content')
    @hasrole('super_admin')
        <form action="{{ route('admin.student-guardian.update', $studentGuardian->id) }}" method="POST"
            ...
            <section class="md:mb-4 max-md:mb-3">
                <div class="flex max-md:flex-col max-md:space-y-1 md:space-x-4 md:items-center">
                    ...
                    <input type="text" name="name" id="name"
                        ...
                        placeholder="Name Lengkap" value="{{ old('name', $studentGuardian->name) }}">
                </div>
                @error('name')
                    ...
                    <select name="student_id" id="student_id" size="-1"
                        ...
                        @foreach ($students as $student)
                            <option value="{{ $student->id }}"
                                {{ $student->id === $studentGuardian->student_id ? 'selected' : '' }}>{{ $student->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <section class="md:mb-4 max-md:mb-3">
                <div class="flex max-md:flex-col max-md:space-y-1 md:space-x-4 md:items-center">
                    ...
                    <input type="number" name="nis" id="nis"
                        ...
                        placeholder="12345678" value="{{ old('nis', $studentGuardian->nis) }}">
                </div>
                @error('nis')
                    <div class="w-full md:pl-52">
                        <small class="text-red-500">{{ $message }}</small>
            ...
            <section class="md:mb-4 max-md:mb-3">
                <div class="flex max-md:flex-col max-md:space-y-1 md:space-x-4 md:items-center">
                    ...
                    <input type="number" name="no_tel" id="no_tel"
                        ...
                        placeholder="08xxxxxxxxxx" value="{{ old('no_tel', $studentGuardian->no_tel) }}">
                </div>
                @error('no_tel')
                    <div class="w-full md:pl-52">
                        <small class="text-red-500">{{ $message }}</small>
